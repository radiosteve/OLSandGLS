%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This program reads annual data, then does OLS and first-order AR model GLS for         %
...
modelOrder=1;     % regression model order (max 9 with this code, but 6 is max for inv(X'X) with Cape Grim data)

plotACF=false;    % plot autocorrelation function of OLS residuals?

plotBestFit=5;    % plot best fit with CI of mean response and PI: 0=no, 1=OLS, 2=ACF, 3=DW, 4=ML, 5=TADW, 6=DW-TADW extrapolated

predictYears=0;   % extend plotting (e.g. 14 years to 2030)

disp('=========================================================================');
% Read data as csv numeric file of two columns: year (AD) and delta-T (deg)
% This file is annual mean delta-T data from Antarctica, Schneider et al (excerpt for 1850-1999)
deltaTdata=csvread('AntarcticaAnnualDT.csv');
[n,~]=size(deltaTdata);
dataMeans=mean(deltaTdata);
...
if randomY
    ...
    disp('Ice core annual Antarctic temperature anomaly, Schneider et. al.');
    disp('Geophys. Res. Lett., 33, L16707, doi:10.1029/2006GL027057.');
    labelY='Temperature anomaly, deg C';
    dataName='AntarcticT';
end
...
if d<d025 || d>d975
    ...
    if plotBestFit==1
        outsideIP=0;
        for i=1:n
            if y(i,1)>bestFitUnEq(i,1)+predInt(i,1) || y(i,1)<bestFitUnEq(i,1)-predInt(i,1)
...
if d<d025 || d>d975
    ...
    if plotBestFit==2
        outsideIP=0;
        for i=1:n
            if y(i,1)>bestFitACF(i,1)+predInt(i,1) || y(i,1)<bestFitACF(i,1)-predInt(i,1)
                outsideIP=outsideIP+1;
            end
        end
...
if d<d025 || d>d975
    ...
    if plotBestFit==3
        outsideIP=0;
        for i=1:n
            if y(i,1)>bestFitDW(i,1)+predInt(i,1) || y(i,1)<bestFitDW(i,1)-predInt(i,1)
                outsideIP=outsideIP+1;
            end
        end
...
if d<d025 || d>d975
    ...
    if plotBestFit==4
        outsideIP=0;
        for i=1:n
            if y(i,1)>bestFitMLE(i,1)+predInt(i,1) || y(i,1)<bestFitMLE(i,1)-predInt(i,1)
                outsideIP=outsideIP+1;
            end
        end
...
if d<d025 || d>d975
    ...
    if plotBestFit==5
        outsideIP=0;
        for i=1:n
            if y(i,1)>bestFitTADW(i,1)+predInt(i,1) || y(i,1)<bestFitTADW(i,1)-predInt(i,1)
                outsideIP=outsideIP+1;
            end
        end
